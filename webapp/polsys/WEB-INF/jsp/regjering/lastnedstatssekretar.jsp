...
> Laste ned data om norske statssekretær
</c:if>
    <c:if test="${en}">
        You are here:
        <a href="<p:url value="/" />">PolSys</a>
        ...
        > Download data about norwegian state secretaries.
    </c:if>
</div>
    <div>
        <p>  <c:if test="${no}"><a href="<m:url value="/en/regjering/lastnedstatssekretar/" />">View this page in English.</a></c:if>
            <c:if test="${en}"><a href="<m:url value="/regjering/lastnedstatssekretar/" />">View this page in Norwegian.</a></c:if>
        </p>
    </div>
   <c:if test="${no}"> <h1>Laste ned data om norske statssekretær</h1>
       <p>	Velg en eller flere regjeringer. Velg deretter på data format å laste ned statssekretærer datasett for de valgte regjeringer. </p>
   </c:if>
    <c:if test="${en}"><h1>Download data about norwegian state secretaries</h1>
        <p> Choose one or more governments. Then select the data set format to download norwegian state secretaries data sets for the selected governments.</p>
    </c:if>



<form action="<p:url value="/regjering/lastnedfilstatssekretar/" />"  method="post">

...
<%-- 
 - Author(s): HVB
 - Copyright NSD
 - Description: Viser oversikt over antall ansatte gitt kommune.
--%>
<c:if test="${no}">
<c:import url="/WEB-INF/jspf/topp_forvaltning.jsp">
    <c:param name="tittelNo" value="${fn:escapeXml(kommune)}, ${fn:escapeXml(fylke)} - Statsansatte - Forvaltningsdatabasen" />
...
> <a href="<p:url value="/data/ansatte/kommune" />">Kommune</a>
> ${fn:escapeXml(kommune)}, ${fn:escapeXml(fylke)}
</c:if>
<c:if test="${en}">
You are here:
<a href="http://www.nsd.uib.no/polsys/en/civilservice/">NSAD</a>
> <a href="http://www.nsd.uib.no/polsys/en/civilservice/stateemployees.html">Employees</a>
> <a href="<p:url value="/data/ansatte/kommune" />">Municipality</a>
> ${fn:escapeXml(kommune)}, ${fn:escapeXml(fylke)}
</c:if>
</div>

<h1>${fn:escapeXml(kommune)}, ${fn:escapeXml(fylke)}</h1>
...
<ul>
<li>Denne siden viser statistikk over statsansatte i ${fn:escapeXml(kommune)}, ${fn:escapeXml(fylke)}.</li>
...
<p:graf tittel="Antall statsansatte (${p:sgln(param.lk)}) i|${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)})"
        aarsverk="${false}"
        total="${total}"
        menn="${mennTotal}"
...
<p:graf tittel="Antall årsverk i staten (${p:sgln(param.lk)}) i|${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)})"
        aarsverk="${true}"
        total="${total}"
        menn="${mennTotal}"
        kvinner="${kvinnerTotal}"
...
<caption>Antall statsansatte (<%--${p:sgln(param.lk)}--%> ${tabellcaption}) i ${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)})</caption>
...
<%-- ============================ ÅRSVERK ===============================  --%>
<table class="zebra sortable">
<caption>Antall årsverk i staten (<%--${p:sgln(param.lk)}--%> ${tabellcaption}) i ${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)})</caption>
...
<%-- ========================== ANDEL ===================================  --%>
<table class="zebra sortable">
<caption>Oversikt over antall og andel statsansatte og årsverk i staten (${p:sgln(param.lk)}) i ${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)})</caption>
...
<p class="tablenote">
Kolonnene "Prosent av ansatte" og "Prosent av årsverk" viser kommunens andel av landets statsansatte/statsårsverk.
...
<table class="zebra sortable">
<caption>Statsansatte (<%--${p:sgln(param.lk)}--%> ${tabellcaption}) per departementsområde i ${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)}), år ${valgtAar}</caption>
...
<%-- =========================== Etat ===================================  --%>
<table class="zebra sortable">
<caption>Statsansatte (<%--${p:sgln(param.lk)}--%> ${tabellcaption}) per etat i ${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)}), år ${valgtAar}</caption>
...
<%-- =========================== Sted ===================================  --%>
<table class="zebra sortable">
<caption>Statsansatte (<%--${p:sgln(param.lk)}--%> ${tabellcaption}) per etat og arbeidssted i ${fn:escapeXml(kommune)} (${fn:escapeXml(fylke)}), år ${valgtAar}</caption>
...
<tr>
<td><a href="<p:url value="/data/ansatte/etat/${fn:escapeXml(a.etatkode)}" />">${fn:escapeXml(a.etatkode)}</a></td>

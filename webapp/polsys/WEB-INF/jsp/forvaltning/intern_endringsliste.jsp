...
<%-- 
 - Author(s): HVB
 - Copyright NSD
 - Description: Viser liste over interne endringer.
--%>
<c:if test="${no}">
<c:import url="/WEB-INF/jspf/topp_forvaltning.jsp">
    <c:param name="tittelNo" value="Interne organisasjonsendringer - Forvaltningsdatabasen" />
    <c:param name="tittelEn" value="Organizational change within ministries - State Administration Database" />
    <c:param name="beskrivelseNo" value="Organisasjonsendringer fordelt på departement og administrativt nivå." />
...
<h1>${en ? "Organizational change distributed on ministries and administrative levels" : "Organisasjonsendringer fordelt på departement og administrativt nivå"}</h1>
...
<c:if test="${feilValg2}">
    <c:if test="${no}"><p><strong>Vennligst velg minst ett nivå.</strong></p></c:if>
...
<ul class="plain">
<c:forEach begin="0" end="${fn:length(nivaaer) - 1}" step="1" var="j">
<c:if test="${!(empty endringsliste[i][j])}">
<li>${fn:length(endringsliste[i][j])} ${fn:escapeXml(nivaaNavn[nivaaer[j]].tekstFlertall)}</li>
</c:if>
</c:forEach>
</ul></li>
</c:if>
...
<c:forEach begin="0" end="${fn:length(nivaaer) - 1}" step="1" var="j">
<c:if test="${!(empty endringsliste[i][j])}">
<h3>${fn:escapeXml(nivaaNavn[nivaaer[j]].tekstFlertall)}</h3>
<ul class="plain">
